...
- **Use of Login/Logout pages**:  
  ...
  3. **Login**: Enter `alice` / `123` → click **Login** → redirected to **Todo List**  
...
|--------|-----|--------------|
| **GET** | `/api/todos` | `curl https://localhost:8099/api/todos` |
| **GET (search)** | `/api/todos?search=test` | `curl "https://localhost:8099/api/todos?search=test"` |
| **POST** | `/api/todos` | `curl -X POST https://localhost:8099/api/todos \ -H "Content-Type: application/json" \ -d '{"title":"Buy milk","description":"2L"}'` |
